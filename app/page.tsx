...
import { listUploadedFiles, removeUploadedFile, uploadFile } from "../lib/files/upload";

type Agent = "General" | "Coder" | "Researcher";
type Attachment = { name: string; url: string; type: string; path?: string };
type Message = { role: "user" | "assistant"; text: string; attachment?: Attachment };

declare global {
  interface Window {
    puter?: {
      auth?: { signIn?: () => Promise<unknown>; signOut?: () => Promise<unknown>; isSignedIn?: () => Promise<boolean>; getUser?: () => Promise<unknown> };
      ai?: { chat?: (prompt: string, options?: Record<string, unknown>) => Promise<unknown> };
    };
  }
}
...
export default function Home() {
  ...
  const [preview, setPreview] = useState<LabFile>();
  ...
  useEffect(() => {
    ...
    listUploadedFiles().then(setFiles).catch(() => {});
    return () => script.remove();
  }, []);

  const login = async () => {
    if (!window.puter?.auth?.signIn) return;
    try { await window.puter.auth.signIn(); setSignedIn(true); } catch (error) { console.error(error); }
  };

  const handleFiles = async (selected: File[]) => {
    if (!selected.length) return;
    if (!signedIn) { await login(); }
    setUploading(true);
    try {
      ...
      if (first?.url) setAttachment({ name: first.name, url: first.url, type: first.mime, path: first.path });
    } catch (error) { console.error(error); alert("อัปโหลดไม่สำเร็จ — ตรวจสอบ Puter Login แล้วลองใหม่"); }
  ...
  const send = async () => {
    ...
    setInput(""); setAttachment(undefined); setBusy(true);
    try {
      if (window.puter?.ai?.chat) {
        const prompt = `[${agent} Agent]\n${text || "วิเคราะห์ไฟล์ที่แนบมา"}${currentAttachment?.path ? `\nPuter file: ${currentAttachment.path}` : ""}`;
        const result = await window.puter.ai.chat(prompt);
        const reply = typeof result === "string" ? result : String((result as { message?: { content?: string } })?.message?.content ?? JSON.stringify(result));
    ...
    } catch { setMessages((m) => [...m, { role: "assistant", text: "เชื่อมต่อ AI ไม่สำเร็จ ลองใหม่อีกครั้งได้ครับ" }]); }
    finally { setBusy(false); }
  };

  const removeFile = async (file: LabFile) => {
    try { await removeUploadedFile(file); setFiles((current) => current.filter((x) => x.id !== file.id)); if (attachment?.path === file.path) setAttachment(undefined); } catch (error) { console.error(error); }
  };
  ...
  return (
    <main className="lab-shell">
      ...
      <aside className={`sidebar ${mobileNav ? "open" : ""}`}>
        ...
        <div className="quick"><small>QUICK ACTIONS</small><button onClick={login}>☁️ <b>เปิด Puter Login</b><em>เชื่อมบัญชี Puter.com</em></button><button>● <b>GitHub MCP</b><em>Repository 77</em></button><button onClick={() => fileRef.current?.click()}>＋ <b>อัปโหลดไฟล์</b><em>รูป · PDF · Office · โค้ด · อื่นๆ</em></button></div>
